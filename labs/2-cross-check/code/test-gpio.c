...
void test_gpio_set_output(int ntrials) { run_test(__FUNCTION__, gpio_set_output, ntrials); }
void test_gpio_set_on(int ntrials)     { run_test(__FUNCTION__, gpio_set_on, ntrials); }
void test_gpio_set_off(int ntrials)    { run_test(__FUNCTION__, gpio_set_off, ntrials); }
...
#   define N 128
    ...
    if(argv[1])
        part = atoi(argv[1]);
    if(part >= 3) 
        panic("test case number too large: %d\n", part);
